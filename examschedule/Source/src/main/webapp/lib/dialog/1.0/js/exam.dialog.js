...
EXAM.Dialog = (function() {
	...
	return {
		...
		create :  function(params){
			...
			sHTML.push('<div class="dialogItem close" title="Close"><img src="lib/dialog/1.0/images/icon_close_24x24.png" onclick="EXAM.Dialog.getDialogByID(\''+sUniqueKey+'\').doDismissCallback()" alt="Close"/></div>');
			sHTML.push('</div>');
			...
			if(!params["hideDialogActions"]) {
				...
				sHTML.push('<input type="button" class="dialogAction okAction" onclick="EXAM.Dialog.getDialogByID(\''+sUniqueKey+'\').doPersistCallback(this)" value="Ok">');
				sHTML.push('</div>');
			}
			sHTML.push('</div>');
			sHTML.push('</div>');
			...
			var obj = {
				...
				setCancelCallback : function(cancelCallbackFunc){
				...
				hideDismiss : function(){
					(oWin.$('#'+this.getID()).find('.close')).remove();
				}
			};
