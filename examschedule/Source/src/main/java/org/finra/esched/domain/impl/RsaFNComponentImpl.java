...
public class RsaFNComponentImpl extends PsComponent {

	private final Logger log = LoggerFactory.getLogger(RsaFNComponentImpl.class);
	
	private PsCmp cmp;

	public RsaFNComponentImpl(PsCmp cmp) {
		super(cmp);
	...
	public PsTriggerDt getTriggerDt(PsFirm firm) {
		...
		log.debug("RsaFNComponentImpl.getLastExamDt(). There are "+(lel!=null ? lel.size(): 0)+" last exams.");
		
		PsLastExam fle = PsLastExam.findCmpByType(lel, CMPNT_TYPE.RSA_FINOP);
