...
################################################################################
#
#  Script Name:  bl_EXAMPLANNING_ExamClose_V1_Driver.nsh
#  Locations:    /FINRA/MEMBER_REG/EXAMPLANNING
...
SUB_APP()
{
    ...
    if [ "${ENV_GROUP}" = "FRED" ] ; then
        #--------------------------------------
        # Create job to run the SQL script
        #--------------------------------------
        
        # First, run the code snippet to decode the encrypted password and place it in a tmp file for use during the actual job run
        
        ######## Code snippet to decode encrypted password and place in file for use during job run
        RESPONSE_FILE="//ny4lxutilp018.finra.org/opt/bmc/storage/blogic/apps/${APP}/${DB_SID}_${DB_USER}.txt"
        echo " \n  After Location  is set  : ${RESPONSE_FILE} "
        getEncryptedPropertyValue "Class://SystemObject${FOLDER_ROOT}/ENVIRONMENT_INSTANCES/${TARGET_ENV}" "DB_PASS"        
        echo "${RESULT}" > ${RESPONSE_FILE}
        ######## End of code snippet

        SOURCE_FOLDER="${COMPONENT_ABBR}_sql_deploy"
        SQL_FILE="deploy_clean.sql"
        SQL_PATH="db_scripts"
        JOB_NAME=`printf "%02d" $jobNum`"-${APP}-${COMPONENT}-${RELEASE}-${TIMESTAMP}-db_deploy-${CURR_TARGET_LOWER}-in-${ENV_LOWER}"
        SUBJECT="SUCCESSFUL DB Deployment : ${JOB_NAME}"
        INSTALL_CMD="${BASE_CMD} rm -f ${LOG_FILE} && touch ${LOG_FILE} && chmod 755 ${LOG_FILE} && \`pwd\`/exec_sql.sh ${SQL_PATH}/${SQL_FILE} ${DB_SERVER} ${DB_USER} ${DB_SID} ${LOG_FILE} >> ${LOG_FILE} 2>&1"
        createCustomSoftwareDeployWithResp "${FOLDER}" "${OSTYPE}" "${SOURCE}/${SOURCE_FOLDER}" "${INSTALL_TYPE}" "${JOB_NAME}" "${JOB_NAME}" "${INSTALL_CMD}" "${UNINSTALLCMD}" "${DB_SERVER}" "${RESPONSE_FILE}"
        jobKey[$jobNum]=${RESULT} ; ((jobNum=jobNum+1))
        jobKey_DB[$jobNum_DB]=${RESULT} ; ((jobNum_DB=jobNum_DB+1))

        #--------------------------------------
        # Create job to send email
        #--------------------------------------
        JOB_NAME=`printf "%02d" $jobNum`"-${APP}-${COMPONENT}-${RELEASE}-${TIMESTAMP}-db_deploy-Email-Log-${ENV_LOWER}"
        SUBJECT="${JOB_NAME}"
        INSTALL_CMD="chmod -R 755  . ; cat ${LOG_FILE} | ${MAIL} -s \"${SUBJECT}\" ${MAIL_TO}"
        createCustomSoftwareDeploy "${FOLDER}" "${OSTYPE}" "${SOURCE}/buildinfo" "${INSTALL_TYPE}" "${JOB_NAME}" "${JOB_NAME}" "${INSTALL_CMD}" "${UNINSTALLCMD}" "${DB_SERVER}" 
        jobKey[$jobNum]=${RESULT} ; ((jobNum=jobNum+1))
        jobKey_DB[$jobNum_DB]=${RESULT} ; ((jobNum_DB=jobNum_DB+1))
    fi
}
...
for envInstance in ${envInstances} ; do
    ...
    if [ "${ENVIRONMENT}" = "${TARGET_ENV}" ] ; then
        ...
        # Get the DB related values from the Property Dictionary
        getEncryptedPropertyValue "${envInstance}" DB_PASS
        DB_PASS="`echo "${RESULT}" | tr -d '\"' | tr -d '[:space:]' | tr -s ',' ' '`"                           
        getPropertyInstanceValue "${envInstance}" DB_USER
        DB_USER="`echo "${RESULT}" | tr -d '\"' | tr -d '[:space:]' | tr -s ',' ' '`"
        getPropertyInstanceValue "${envInstance}" DB_SID
        DB_SID="`echo "${RESULT}" | tr -d '\"' | tr -d '[:space:]' | tr -s ',' ' '`"
        ...
        for targetType in ${SERVER_TYPES} ; do
            ...
            if [ "${targetType}" = "DB" ] ; then
                DB_SERVER=${DB_SERVER},${target}
            fi
        ...
        #create_batch_job jobKey_DB      "#DB-DEPLOY-${ENV_LOWER}" "${jobGroupId}"
